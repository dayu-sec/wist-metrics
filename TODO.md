...
已下沉至此，`wist-agentd` 只做再导出。以下是 DB 指标采集（Postgres / MySQL）的剩余工作。

## 待落地

1. **加 `sqlx` 依赖（feature 门控）**
   - `sqlx` 用 `default-features = false` + `runtime-tokio-rustls` + `tls-rustls-ring`（或等价）驱动；
   - 拆 `postgres` / `mysql` 两个 feature，默认关闭，保证 `wist-agentd` 默认构建不碰 sqlx；
   - DB 驱动只在本 crate，不进 `wist-agentd` 依赖图。
...
1. **DSN 来源**：discovery 只给「端口 + 进程」，拿不到用户名/密码。
   - 方案 A：`wist-agentd` config 新增 `db_metrics` 段（每目标一个 DSN 或全局连接串）；
   - 方案 B：DSN 由 center 下发（计划内携带）——依赖 W4 capability 协商已就绪。
   - 当前 `wist-agentd` 无任何 dsn/database 配置，属全新添加。
